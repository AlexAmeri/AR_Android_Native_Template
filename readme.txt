...
(2) The Metaio View is loaded (the "ARA" class is the Activity which holds the main app content). The Metaio API has a configuration file oboard the client device called "TrackingData/MarkerlessFast.xml" which is used to define target images for the API engine to recognize, as well as general settings. 
